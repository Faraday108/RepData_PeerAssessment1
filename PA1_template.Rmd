...
To answer this first question, we want to look at a subset of the data where we ignore the `NA` values. Note that this will remove `r sum(is.na(data$steps))` rows.  
...
This process shows that the average number of daily steps is `r format(summary_daily_steps$mean[[1]], scientific = FALSE)` and the median number of daily steps is `r format(summary_daily_steps$median[[1]], scientific = FALSE)`. 
...
```
From the graph, the interval with the highest mean number of steps is `r highest_mean_interval[1]`.
